...
requests.post('http://localhost:3355/send-message-to-client', json={'message': 'Starting import data from csv into database.'})
...
with open(csv_file_path, 'r') as csv_file:
    ...
    for row in reader:
        ...
        if len(rows_to_insert) % batch_size == 0 or current_row == total_rows:
            ...
            requests.post('http://localhost:3355/send-message-to-client', json={'message': f'TRows Total {total_rows} Rows'})
            requests.post('http://localhost:3355/send-message-to-client', json={'message': f'TBatch Total {total_batches} Batch'})
            requests.post('http://localhost:3355/send-message-to-client', json={'message': f'CBatch Current batch : {current_batch} of {total_batches} inserted'})
            requests.post('http://localhost:3355/send-message-to-client', json={'message': f'Estimated remaining time: {estimated_remaining_time:.2f} minutes'})
