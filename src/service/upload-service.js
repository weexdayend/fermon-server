...
const uploadService = {
    ...
    processUploadedFileBulanF5: async (file, res, next) => {
        try {
            await db.$connect();
            const pythonScript = spawn('/root/venv/bin/python3', ['src/service/proses_f5.py', file.path]);
    ...
    processUploadedFileBulanF6: async (file, res) => {
        try {
            await db.$connect();
            const pythonScript = spawn('/root/venv/bin/python3', ['src/service/proses_f6.py', file.path]);
